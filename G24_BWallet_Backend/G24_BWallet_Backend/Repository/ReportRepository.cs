...
namespace G24_BWallet_Backend.Repository
{
    public class ReportRepository : IReportRepository
    {
        ...
        public async Task<Report> createReport(int receiptID, int userID, string reason)
        {
            ...
            Receipt receipt = context.Receipts.Include(r => r.UserDepts).Where(r => r.Id == receiptID).FirstOrDefault();
        ...
        public async Task<Report> responeReport(int reportId, int status)
        {
            ...
            Receipt receipt = context.Receipts.Include(r => r.UserDepts).Where(r => r.Id == report.ReceiptId).FirstOrDefault();
            switch (status)
            {
                case 1://accept
